...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    try {
      const userRes = await fetch('http://user-service:3003/api/users/internal/profile', {
        method: 'POST',
      ...
      if (!userRes.ok) {
        console.error('[AUTH] Failed to sync profile to user-service. Status:', userRes.status);
      } else {
        console.log(`[AUTH] Profile synced to user-service for user: ${user.user_id}`);
      }
    } catch (syncErr) {
      console.error('[AUTH] Error syncing profile to user-service:', syncErr.message);
    }
